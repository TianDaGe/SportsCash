...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a SportsCash address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no SportsCash: URI
...
{
    // Convert sportscash:// to sportscash:
    //
    //    Cannot handle this later, because sportscash:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open sportscash.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SportsCash.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for SportsCash.lnk
...
{
    return GetAutostartDir() / "sportscash.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a sportscash.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=SportsCash\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the sportscash app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add sportscash app to startup item list
