...
#if defined(NDEBUG)
#error "SportsCash cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    } else if (nHeight > 0 && nHeight <= 2000) {
        nSubsidy = 1 * COIN;
    } else if (nHeight > 2000 && nHeight <= 1144857) {
        nSubsidy = 7 * COIN;
    } else if (nHeight > 1144857 && nHeight <= 2744857) {
        nSubsidy = 5 * COIN;
    } else if (nHeight > 2744857 && nHeight <= 5411523) {
        nSubsidy = 3 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from SportsCash
...
{
    RenameThread("sportscash-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // SportsCash: We use certain sporks during IBD, so check to see if they are
